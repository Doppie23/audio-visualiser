...
pub fn Eq(fft_size: usize, smoothing: f32) type {
    return struct {
        ...
        pub fn init() Self {
            comptime std.debug.assert(fft_size % 2 == 0);

            const in: [fft_size]f32 = undefined;
            const smoothed: [fft_size / 2]f32 = .{1} ** (fft_size / 2);

            // https://numpy.org/doc/stable/reference/generated/numpy.hanning.html
            const hann_table = comptime blk: {
                @setEvalBranchQuota(fft_size * 4);
